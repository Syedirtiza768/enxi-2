...
export default function ItemImportPage() {
  ...
  const handleImport = async () => {
    ...
    try {
      setProgress(20)
      const response = await apiClient('/api/inventory/items/bulk-import', {
        method: 'POST',
        body: formData,
      })
                ...
                <div className="space-y-4">
                        ...
                        {result.errors.map((error, idx) => (
                          <Alert key={idx} variant="destructive" className="py-2">
                            <AlertDescription className="text-sm">
                              Row {error.row}: {error.name || error.code || 'Unknown'} - {error.error}
                            </AlertDescription>
                          </Alert>
                        ))}
                      </div>
                  ...
                  {result.successCount > 0 && (
                    <Alert>
                      ...
                      <AlertDescription>
                        Successfully imported {result.successCount} items. Redirecting to items list...
                      </AlertDescription>
                    </Alert>
                  )}
                </div>
